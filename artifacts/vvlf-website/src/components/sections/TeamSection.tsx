    ...
    <section className="py-24 bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-28">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {board.map((member, i) => (
              <FadeIn key={member.name} delay={i * 0.1}>
                <div className="group text-center bg-white rounded-2xl p-6 border border-slate-100 hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300">
                  <div className="relative w-32 h-32 mx-auto rounded-full overflow-hidden mb-5 border-4 border-white shadow-xl shadow-slate-200/50">
                    ...
                    <div className="absolute inset-0 bg-navy/70 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                      <a href={member.linkedin} className="w-9 h-9 rounded-full bg-[#0A66C2] text-white flex items-center justify-center hover:scale-110 transition-transform">
        ...
        <div>
          <FadeIn>
            <div className="text-center max-w-2xl mx-auto mb-16">
              <h2 className="text-primary font-bold tracking-wider text-sm uppercase mb-2">Core Team</h2>
          ...
          <div className="flex flex-wrap justify-center gap-10">
            {team.map((member, i) => (
              <FadeIn key={member.name} delay={i * 0.1}>
                <div className="group bg-white rounded-3xl p-8 border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 w-full sm:w-96 flex flex-col items-center text-center">
                  <div className="relative w-40 h-40 mb-6 rounded-full overflow-hidden border-4 border-white shadow-xl shrink-0">
                    <img
                      src={`${import.meta.env.BASE_URL}images/${member.photo}`}
                      alt={member.name}
                      className="w-full h-full object-cover object-top group-hover:scale-110 transition-transform duration-500"
        ...
        <div>
          <FadeIn>
            <div className="text-center max-w-2xl mx-auto mb-16">
              <h2 className="text-primary font-bold tracking-wider text-sm uppercase mb-2">Mentor Network</h2>
              <h3 className="text-3xl md:text-4xl font-display font-bold text-navy">Industry Mentors</h3>
              <p className="text-slate-500 mt-3">Domain experts guiding our startups across every sector.</p>
            </div>
          </FadeIn>
            ...
            {mentors.map((mentor, i) => (
              <FadeIn key={i} delay={i * 0.07}>
                <div className="group bg-white rounded-2xl p-6 border border-slate-100 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 flex items-center gap-5">
                  <div className={`w-14 h-14 rounded-full flex items-center justify-center font-bold text-lg shrink-0 ${mentor.color} group-hover:scale-110 transition-transform`}>
                    {mentor.initials}
                  </div>
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5 mb-0.5">
                      <Star className="w-3.5 h-3.5 text-amber-400 fill-amber-400 shrink-0" />
                      <h5 className="font-bold text-navy text-sm truncate">{mentor.name}</h5>
                    </div>
        ...
        <div>
          <FadeIn>
            <div className="text-center max-w-2xl mx-auto mb-16">
              <h2 className="text-primary font-bold tracking-wider text-sm uppercase mb-2">Advisory & Partners</h2>
              <h3 className="text-3xl md:text-4xl font-display font-bold text-navy">Our Strategic Partners</h3>
            ...
            {advisors.map((a, i) => (
              <FadeIn key={i} delay={i * 0.1}>
                <div className="group bg-white rounded-2xl p-8 border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col items-center text-center">
                  <div className={`w-16 h-16 rounded-full flex items-center justify-center font-bold text-xl mb-4 ${a.color} group-hover:scale-110 transition-transform`}>
                    {a.initials}
                  </div>
                  <h5 className="font-bold text-navy text-base mb-1">{a.name}</h5>
